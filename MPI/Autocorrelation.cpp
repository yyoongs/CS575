...
main( int argc, char *argv[ ] )
{
	...
	// bigsignal 배열 생성 numelements + maxshift size
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXSHIFTS, MPI_FLOAT, dst, 'S', MPI_COMM_WORLD );
		}
	}
	else
	{
		MPI_Recv( PPSignal, PPSize+MAXSHIFTS, MPI_FLOAT, BOSS, 'S', MPI_COMM_WORLD, &status );
	}
